# 📚 FleetFlow: What We've Built & What You Need to Know

## 🎯 The Big Picture: What Is FleetFlow?
...
# - Is vehicle_id a valid UUID? ✅
# - Is data a list of TelemetryPoints? ✅
...
# Counts harsh events: g_force < -0.4 (braking) → 1 event
# Counts rapid accel: g_force > 0.4 (accel) → 0 events
# Score = 100 - (1 + 0) * 5 = 95
...
## 📁 Code Organization (What Each File Does)

```
backend/src/

├── main.py
│   └── FastAPI service (the API server)
│       ├── Receives telemetry data
│       ├── Stores in database
│       ├── Publishes to RabbitMQ
│       └── Returns results when queried
│
├── worker.py
...
│       ├── TripPayload (what API accepts)
│       ├── TelemetryPoint (single sensor reading)
│       └── DriverScore (result format)
│
└── analytics.py
    └── Business logic
        └── calculate_safety_score() (scoring algorithm)
```

---

## 🎯 What You Should Know About This Codebase
...
```
❌ Wrong:  API → Queue (huge JSON) → Worker
✅ Right:  API → Database (huge JSON)
           API → Queue (just ID)
...
# If invalid fields → raises validation error automatically
# If missing fields → raises validation error automatically
```
...
Sends telemetry → waits for score → verifies end-to-end works.

---

## 🧠 Key Learning Points (The Big Ideas)
...
## 🎓 Python Concepts You'll Encounter
...
You're now equipped with modern Python knowledge! 🚀

### 1. The End-to-End Workflow (The "Claim Check" Pattern)

This is the most important architectural concept. Imagine a coat check at a theater: you don't carry your heavy coat into the show; you give it to a clerk, get a tiny ticket (the "claim"), and use that ticket later to get your coat back.

- **Step 1 (The API):** A car sends 5,000 data points. The API saves this "heavy" data into **PostgreSQL** immediately.
- **Step 2 (The Ticket):** The API generates a `trip_id` (the ticket) and puts it into **RabbitMQ**.
- **Step 3 (The Response):** The API tells the car "202 Accepted" and gives it the `trip_id`. The car is now free to go.
- **Step 4 (The Worker):** The Worker sees the `trip_id` in RabbitMQ, goes to the database, "claims" the 5,000 points, calculates the score, and saves the result.

**Why?** Because calculating a score for 5,000 points takes time. If the API did the calculation while the car was waiting, the connection might time out.

---

### 2. Async Programming (`async` and `await`)

In traditional Python, if you ask a database for data, the whole program **stops and waits** until the database answers. This is "Blocking."

In **Async Programming**, we use a "Non-blocking" approach:

- **`async def`**: Tells Python "this function might have to wait for something (like a database or network)."
- **`await`**: Tells Python "I'm waiting for this specific task. While I wait, feel free to go handle other incoming API requests."
...
async def init_db():
    global db_pool
    # We 'await' the connection. Python can do other things while the network
    # handshake with PostgreSQL happens.
    db_pool = await psycopg.AsyncConnection.connect(DATABASE_URL)
```

---

### 3. `asynccontextmanager` (The Lifespan Pattern)

In FastAPI, we need a way to "set things up" when the server starts and "clean things up" when it stops. We use a **Context Manager** for this.

Think of it like a "sandwich":

1.  **Top Bread (Startup):** Connect to DB, connect to RabbitMQ.
2.  **The Filling (`yield`):** This is where the actual API runs and handles requests.
3.  **Bottom Bread (Shutdown):** Close connections so we don't leak memory.
...
In models.py, we use Pydantic classes. This is "Standard Practice" for modern Python.

Instead of manually checking if `speed_kmh` is a number, Pydantic does it for you. If a car sends a string instead of a number, FastAPI will automatically return a `422 Unprocessable Entity` error before your code even runs.

```python
class TelemetryPoint(BaseModel):
    speed_kmh: float  # Pydantic ensures this is ALWAYS a float
```

---

### 5. Why Docker is the "Standard"

As you saw, running Python directly on Windows can be a nightmare because of how Windows handles "Event Loops" (the engine that runs `async` code).

- **The Hoop:** Windows uses `ProactorEventLoop` which doesn't like PostgreSQL.
- **The Docker Solution:** Docker runs a tiny **Linux** kernel. Linux uses `SelectorEventLoop` which is the gold standard for high-performance networking. By moving to Docker, we stopped fighting the OS and started using the environment the code was designed for.

### Summary of the Stack

- **FastAPI**: The fast, async web "front door."
- **PostgreSQL**: The "long-term memory" (stores the heavy data).
- **RabbitMQ**: The "to-do list" (tells the worker what to process).
- **Worker**: The "brain" (does the heavy math in the background).
